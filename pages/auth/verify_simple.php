...
if ($email_param && filter_var($email_param, FILTER_VALIDATE_EMAIL)) {
    try {
        ...
        if ($user && !$user['is_verified']) {
            ...
            // Send OTP email
            $mail_sent = sendOTPEmail($email_param, $user['full_name'], $otp_code);

            if ($mail_sent) {
                $otp_sent = true;
                $email = $email_param;
                $countdown_time = $expires_in_minutes * 60; // Convert to seconds
                $success = 'We\'ve sent a verification code to your email. Check your inbox.';
                $step = 'verify';
            } else {
                $error = 'Could not send verification code. Please enter your email to request one.';
                $step = 'request';
            }
        }
    } catch (Exception $e) {
    ...
    if (!filter_var($request_email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            ...
            if ($user && !$user['is_verified']) {
                ...
                // Send OTP email
                $mail_sent = sendOTPEmail($request_email, $user['full_name'], $otp_code);
