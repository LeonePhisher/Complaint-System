...
$error = '';
$success = '';
$verifying = false; // new flag to show verifying message

if ($token) {
    $verifying = true; // tell JS to show “Verifying Email…” spinner

    try {
        // Fetch student with valid token
        $stmt = db()->prepare("
            SELECT * FROM users 
            WHERE verification_token = ? 
              AND verification_expires > NOW()
        ...
        if ($student) {
            ...
            // Only show error if token exists in URL but is invalid
            $error = 'Invalid or expired verification link.';
        }
    } catch (PDOException $e) {
        error_log("Verification error: " . $e->getMessage());
        ...
        if ($student) {
            if ($student['is_verified']) {
                ...
                $stmt = db()->prepare("
                    ...
                    WHERE id = ?
                ");
                $stmt->execute([$token, $expires_at, $student['id']]);
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
    const verifying = <?php echo $verifying ? 'true' : 'false'; ?>;
